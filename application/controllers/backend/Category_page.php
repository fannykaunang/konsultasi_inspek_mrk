	...
	public function index()
	{
		if ($this->user_model->CheckSession() == 1) {
			...
			/*
					 ...
					 */
			/*role*/
			$this->load->view("backend/category-page", $data);
				...
				if ($do = $this->input->post('do')) {
					switch ($do) {
						case "CategoryCreate":
							...
							$query = $this->db->insert('categorypage', $data);
							if ($query) {
						...
						case "CategoryDelete":
							...
							$query = $this->db->where('IdCategoryPage', $IdCategory);
							...
							if ($query->num_rows() > 0) {
								...
								$query = $this->db->delete('categorypage');
								if ($query) {
									$respon = array(
										'status' => 'sukses',
						...
						case "CategoryUpdate":
							...
							$query = $this->db->where('IdCategory', $IdCategory);
							$query = $this->db->update('categorypage', $data);
							if ($query) {
								$respon = array(
									'status' => 'sukses',
...
/* End of file Category_page.php */
/* Location: ./application/controllers/backend/Category_page.php */