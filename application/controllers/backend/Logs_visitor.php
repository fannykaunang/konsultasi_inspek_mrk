...
class Logs_visitor extends CI_Controller
{
	...
	public function index()
	{
		if ($this->user_model->CheckSession() == 1) {
			...
			$data['page_title'] = 'Logs Pengunjung - Inspektorat Kabupaten Merauke';
			$data['logsmenu'] = 'active';
			$data['logsvisitorsubmenu'] = 'active';
			/*role*/
			...
			/*role*/
			$this->load->view("backend/logs-visitor", $data);
	...
	function LogsVisitorList()
	{
		if ($this->user_model->CheckSession() == 1) {
			if ($this->input->is_ajax_request() == true) {
				...
				$query = $this->db->query("
						SELECT IdLog, IPAddress, TimeStamp, Browser, AgentString, Platform, Version
						FROM $table
						");
				...
				if (!empty($requestData['search']['value'])) {
					...
					$sql = " SELECT IdLog, IPAddress, TimeStamp, Browser, AgentString, Platform, Version ";
					$sql .= " FROM $table ";
