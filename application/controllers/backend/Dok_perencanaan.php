...
class Dok_perencanaan extends CI_Controller
{

	public function index()
	{
		if ($this->user_model->CheckSession() == 1) {
			$data['page_title'] = 'Dokumen Perencanaan - Inspektorat Kabupaten Merauke';
			$data['dokumen_menu'] = 'active';
			$data['perencanaan_submenu'] = 'active';
			/*role*/
			...
			/*
									   ...
									   */
			/*role*/
			$this->load->view("backend/dok-perencanaan", $data);
	...
	function SliderList()
	{
		if ($this->user_model->CheckSession() == 1) {
			if ($this->input->is_ajax_request() == true) {
				...
				$query = $this->db->query("
						SELECT IdPerencanaan, Caption, Fullpath, Basename, Extension, Filesize, Time, FlagPublish
						FROM $table
						");
				...
				if (!empty($requestData['search']['value'])) {
					...
					$sql = " SELECT IdPerencanaan, Caption, Fullpath, Basename, Extension, Filesize, Time, FlagPublish ";
					$sql .= " FROM $table ";
				...
				if ($query->num_rows() > 0) {
					...
					foreach ($query->result() as $row) {
						$data[] = array(
							...
							'Option' => '<button onclick="FileEdit(' . $row->IdPerencanaan . ')" class="btn btn-sm btn-icon btn-primary btn-round" ' . $RoleFileUpdate . '>
												<i class="icon wb-pencil"></i> 
											</button>
											<button onclick="FileDelete(' . $row->IdPerencanaan . ')" class="btn btn-sm btn-icon btn-danger btn-round" ' . $RoleFileDelete . '>
				...
				if ($do = $this->input->post('do')) {
					switch ($do) {
						case "FileDelete":
							...
							$query = $this->db->where('IdPerencanaan', $IdPerencanaan);
							$query = $this->db->get('dok_perencanaan');
							if ($query->num_rows() > 0) {
								$row = $query->row();
								...
								$query = $this->db->where('IdPerencanaan', $IdPerencanaan);
								$query = $this->db->delete('dok_perencanaan');
								if ($query) {
									$respon = array(
										'status' => 'sukses',
										'message' => 'Hapus file'
						...
						case "FileUpdate":
							...
							$query = $this->db->where('IdPerencanaan', $IdPerencanaan);
							$query = $this->db->update('dok_perencanaan', $data);
							if ($query) {
								$respon = array(
									'status' => 'sukses',
		...
		if ($this->user_model->CheckSession() == 1) {
			...
			if (!$this->upload->do_upload()) {
				...
				foreach ($result as $row) {
					...
					$query = $this->db->insert('dok_perencanaan', $data);
