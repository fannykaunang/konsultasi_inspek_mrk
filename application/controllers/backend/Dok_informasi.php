...
class Dok_informasi extends CI_Controller
{

	public function index()
	{
		if ($this->user_model->CheckSession() == 1) {
			$data['page_title'] = 'Dokumen permohonan informasi - Inspektorat Kabupaten Merauke';
			$data['dokumen_menu'] = 'active';
			$data['permohonan_informasi_submenu'] = 'active';
			/*role*/
			...
			/*
									   ...
									   */
			/*role*/
			$this->load->view("backend/dok-informasi", $data);
	...
	function SliderList()
	{
		if ($this->user_model->CheckSession() == 1) {
			if ($this->input->is_ajax_request() == true) {
				...
				$query = $this->db->query("
						SELECT IdInformasi, Caption, Fullpath, Basename, Extension, Filesize, Time, FlagPublish
						FROM $table
						");
				...
				if (!empty($requestData['search']['value'])) {
					...
					$sql = " SELECT IdInformasi, Caption, Fullpath, Basename, Extension, Filesize, Time, FlagPublish ";
					$sql .= " FROM $table ";
				...
				if ($query->num_rows() > 0) {
					...
					foreach ($query->result() as $row) {
						$data[] = array(
							...
							'Option' => '<button onclick="FileEdit(' . $row->IdInformasi . ')" class="btn btn-sm btn-icon btn-primary btn-round" ' . $RoleFileUpdate . '>
												<i class="icon wb-pencil"></i> 
											</button>
											<button onclick="FileDelete(' . $row->IdInformasi . ')" class="btn btn-sm btn-icon btn-danger btn-round" ' . $RoleFileDelete . '>
		...
		if ($this->user_model->CheckSession() == 1) {
			if ($this->input->is_ajax_request() == true) {
				if ($do = $this->input->post('do')) {
					switch ($do) {
						case "FileDelete":
							$IdInformasi = $this->input->post('IdInformasi');
							$query = $this->db->where('IdInformasi', $IdInformasi);
							$query = $this->db->get('dok_informasi');
							if ($query->num_rows() > 0) {
								$row = $query->row();
								...
								$query = $this->db->where('IdInformasi', $IdInformasi);
								$query = $this->db->delete('dok_informasi');
								if ($query) {
									$respon = array(
										'status' => 'sukses',
										'message' => 'Hapus file'
						...
						case "FileUpdate":
							...
							$query = $this->db->where('IdInformasi', $IdInformasi);
							$query = $this->db->update('dok_informasi', $data);
							if ($query) {
								$respon = array(
									'status' => 'sukses',
		...
		if ($this->user_model->CheckSession() == 1) {
			...
			if (!$this->upload->do_upload()) {
				...
				foreach ($result as $row) {
					...
					$query = $this->db->insert('dok_informasi', $data);
