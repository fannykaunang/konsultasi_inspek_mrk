...
<body class="site-menubar-unfold site-menubar-keep">
  ...
  <div class="page">
    <div class="page-header">
        ...
        <li class="active">Edit Direktori</li>
      </ol>
      <h1 class="page-title">Edit Direktori Statis</h1>
			...
			<div class="panel">			
				...
				if($RoleNewsUpdate == 'yes'){					
					if($editdirectory !== null){					
						foreach($editdirectory as $row){ 
						?>
						<form class="form-horizontal" id="FormDirectoryEdit" name="FormDirectoryEdit">
								<div class="form-group">
									<label for="judul" class="col-md-2 control-label">Metadata</label>
									<div class="col-md-10">
										 <input type='text' class="form-control" value="Penulis: <?=$authordirectory;?>, Terakhir update: <?=DateTimeIndo($row->UpdatedDirectory);?>, Oleh: <?=$row->UpdatedBy;?>" disabled >
									</div>
								</div>
								<div class="form-group">
									<label for="judul" class="col-md-2 control-label">Judul Direktori</label>
									<div class="col-md-10">
										 <textarea class="form-control" id="TitleDirectory" name="TitleDirectory" title="Judul Directory" data-toggle="tooltip" placeholder="Judul direktori"><?=$row->TitleDirectory;?></textarea>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-md-3">
										<select class="form-control show-menu-arrow" data-plugin="selectpicker" name="IdCategory" id="IdCategory" data-toggle="tooltip" title="Kategori">
											<?php
												foreach($categorydirectory as $category){
													if($category->IdCategory == $row->IdCategoryDirectory){$sel = 'selected';}else{$sel='';}
													?>
								...
								<div class="form-group">
									...
									<div class="col-md-3">
										<div class="input-group">
											<input type='text' class="form-control" id="CreatedDirectory" data-toggle="tooltip" title="Tanggal" placeholder="Tanggal" name="CreatedDirectory" value="<?=DateIndo($row->CreatedDirectory);?>" data-plugin="datepicker">
											<span class="input-group-addon"><i class="icon wb-calendar"></i></span>
										</div>
									</div>				
								</div>
								<div class="form-group">
									<label for="IsiPage"  class="col-md-2 control-label">Isi Direktori</label>
									<div class="col-md-10">
										<textarea class="form-control" id="ContentDirectory" name="ContentDirectory" data-toggle="tooltip" title="Isi direktori" rows="10"><?=$row->ContentDirectory;?></textarea>
									</div>				
								</div>
								...
								<div class="form-group">
									...
									<div class="col-md-9">
											<?php
												if($row->FlagDate=='1'){$FlagDate='checked';}else{$FlagDate='';}
												if($row->FlagPublish=='1'){$FlagPublish='checked';}else{$FlagPublish='';}
												if($row->FlagComment=='1'){$FlagComment='checked';}else{$FlagComment='';}
											?>
								...
								<div class="form-group">
									...
									<div class="col-md-8">
										<input type='text' class="form-control" id="Thumbnail" data-toggle="tooltip" title="Gambar" placeholder="Gambar cuplikan direktori" name="Thumbnail" value="<?=$row->Thumbnail;?>">
									</div>
									<div class="col-md-1">
								...
								<input type="hidden" name="IdDirectory" id="IdDirectory" value="<?=$row->IdDirectory;?>"/>
...
<div class="modal fade" id="DialogGallery" aria-hidden="true" aria-labelledby="exampleGrid"
 ...
 <script>
	...
	$(function(){
		//------------------------------------------------------------------------------------//
		$("#FormDirectoryEdit").find("#TitleDirectory").focus();
		//------------------------------------------------------------------------------------//
		tinymce.init({
		...
		$("#FormDirectoryEdit").submit(function(e){
			e.preventDefault();
			if($("#FormDirectoryEdit").valid()){				
				//////////////////////
				bootbox.dialog({
					...
					buttons: {				
						...
						main: {
							...
							callback: function() {
									var form = $("#FormDirectoryEdit").serialize();
									$.ajax({
										url: '<?=base_url();?>backend/directory_information/ajax',
										type: 'POST',
										dataType: 'json',
										data:'do=DirectoryUpdate&'+form,
										success: function(respon){
											if(respon.status=='sukses'){																			
												$.growl({title:respon.status,message: respon.message});
												setTimeout(function(){window.location='<?=base_url();?>backend/directory_information';},1000);
											}											
										},
										timeout: 20000,
										error: function(){
	...
	function SelectImage(Fullpath){
		$("#FormDirectoryEdit").find("#Thumbnail").val(Fullpath);
