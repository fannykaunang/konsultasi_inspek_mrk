					...
					if($editnews !== null){					
						...
						<form class="form-horizontal" id="FormNewsEdit" name="FormNewsEdit">
								<div class="form-group">
									...
									<div class="col-md-10">
										 <input type='text' class="form-control" value="Penulis: <?=$authornews;?>, Terakhir update: <?=DateTimeIndo($row->UpdatedNews);?>, Oleh: <?=$row->UpdatedBy;?>" disabled >
									</div>
								</div>
								<div class="form-group">
									<label for="judul" class="col-md-2 control-label">Judul Berita</label>
									<div class="col-md-10">
										 <textarea class="form-control" id="TitleNews" name="TitleNews" title="Judul Berita" data-toggle="tooltip" placeholder="Judul berita"><?=$row->TitleNews;?></textarea>
									</div>
								</div>
										...
										<select class="form-control show-menu-arrow" data-plugin="selectpicker" name="IdCategory" id="IdCategory" data-toggle="tooltip" title="Kategori">
											<?php
												foreach($categorynews as $category){
													if($category->IdCategory == $row->IdCategory){$sel = 'selected';}else{$sel='';}
													?>
								...
								<div class="form-group">
									...
									<div class="col-md-3">
										<div class="input-group">
											<input type='text' class="form-control" id="CreatedNews" data-toggle="tooltip" title="Tanggal" placeholder="Tanggal" name="CreatedNews" value="<?=DateIndo($row->CreatedNews);?>" data-plugin="datepicker">
								...
								<div class="form-group">
									...
									<div class="col-md-10">
										<textarea class="form-control" id="ContentNews" name="ContentNews" data-toggle="tooltip" title="Isi berita" rows="10"><?=$row->ContentNews;?></textarea>
									</div>				
								</div>
								...
								<div class="form-group">
									...
									<!--<div class="col-md-8">
										<input type='text' class="form-control" id="Thumbnail" data-toggle="tooltip" title="Gambar" placeholder="Gambar cuplikan berita" name="Thumbnail" value="<?=$row->Thumbnail;?>">
									</div>
									-->
									...
									<div class="col-md-5">
										...
										<input type='hidden' class="form-control" id="Thumbnail" data-toggle="tooltip" title="Gambar" placeholder="Gambar cuplikan berita" name="Thumbnail" value="<?=$row->Thumbnail;?>">
								...
								<input type="hidden" name="IdNews" id="IdNews" value="<?=$row->IdNews;?>"/>
...
<div class="modal fade" id="DialogGallery" aria-hidden="true" aria-labelledby="exampleGrid"
 ...
 <script>
	...
	$(function(){
		//------------------------------------------------------------------------------------//
		$("#FormNewsEdit").find("#TitleNews").focus();
		//------------------------------------------------------------------------------------//
		tinymce.init({
						...
						main: {
							...
							callback: function() {
									...
									$.ajax({
										...
										type: 'POST',
										dataType: 'json',
										data:'do=NewsUpdate&'+form,
										success: function(respon){
											if(respon.status=='sukses'){																			
												$.growl({title:respon.status,message: respon.message});
												setTimeout(function(){window.location='<?=base_url();?>backend/news';},1000);
											}											
										},
										timeout: 20000,
										error: function(){
	...
	function SelectImage(Fullpath){
		$("#FormNewsEdit").find("#ImgThumbnail").attr("src", "<?=base_url();?>"+Fullpath);
		$("#FormNewsEdit").find("#Thumbnail").val(Fullpath);
