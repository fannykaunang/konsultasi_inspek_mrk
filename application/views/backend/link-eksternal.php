...
<body class="site-menubar-unfold site-menubar-keep">
  ...
  <div class="page animsition">
    <div class="page-header">
        ...
        <li class="active">Pengaturan Link terkait</li>
      </ol>
      <h1 class="page-title">Link Terkait</h1>
		<div class="page-header-actions">
				<?php
                    $displayButton = ($totalpage == 8 || $totalpage > 8) ? "display:none" : "display:inline-block";
                ?>
			<button style="<?php echo $displayButton; ?>" class="btn btn-sm btn-icon btn-inverse" data-toggle="tooltip" data-original-title="Unggah Link" onclick="SliderCreate();">
  ...
  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header">
			...
			<h4 class="modal-title" id="myModalLabel">Unggah Link</h4>
		  </div>
		  <div class="modal-body">
				
			  	<form class="form-horizontal" name="FormUploadFile" id="FormUploadFile" method="POST" enctype="multipart/form-data" target="IFrameUploadFile">
					
						<div class="form-group">
					    <label class="col-sm-3 control-label">Nama</label>
					    <div class="col-sm-9">
					     	<input type="text" name="Caption" id="Caption" placeholder="Nama" class="form-control tooltip-right" title="Judul"/>
					    </div>
					  </div>
						<div class="form-group">
					    <label class="col-sm-3 control-label">Link</label>
					    <div class="col-sm-9">
					     	<textarea type="text" name="Description" id="Description" placeholder="paste link disini " class="form-control tooltip-right" title="Paste Link disini"/></textarea>
					    </div>
					  </div>
						<div class="form-group">
...
<div class="modal fade modal-fade-in-scale-up" id="DialogFileEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			<h4 class="modal-title" id="myModalLabel">Update Link</h4>
		  </div>
		  <div class="modal-body">
			  	...
			  	<form class="form-horizontal" name="FormFileEdit" id="FormFileEdit" method="POST">
					    ...
					    <div class="col-sm-9">
					     	<input type="text" name="Caption" id="Caption" placeholder="Judul Link" class="form-control tooltip-right" title="Judul dokumen"/>
					    </div>
					  </div>
						<div class="form-group">
					    <label class="col-sm-3 control-label">Link</label>
 ...
 <script>
	...
	$(function(){
		...
		var MyTable = $("#MyTable").dataTable({
			...
			"bServerSide": true,			
			"ajax": {
				"url": "<?=base_url();?>backend/link_eksternal/SliderList",
				"type": "POST",
				"data":{
		...
		$("#FormFileEdit").submit(function(e){
			...
			if($("#FormFileEdit").valid()){				
				//////////////////////
				bootbox.dialog({
					message: "Apakah anda yakin akan menyimpan link ini?",
					title: "Konfirmasi",
					buttons: {				
						danger: {
							label: "No",
						...
						main: {
							...
							callback: function() {
									...
									$.ajax({
										url: '<?=base_url();?>backend/link_eksternal/ajax',
										type: 'POST',
										dataType: 'json',
 ...
 //------------------------------------------------------------------------------------//
            ...
            //------------------------------------------------------------------------------------//
	//------------------------------------------------------------------------------------//	
	function FileDelete(IdLink){
		bootbox.dialog({
			message: "Anda yakin akan menghapus Link ini?",
			title: "Konfirmasi",
			buttons: {
				danger: {
					label: "No",
			...
			success: function(respon) {
				...
				if(respon.status=='sukses'){
					$.each(respon.data, function(index, element){
						...
						$("#FormFileEdit").find("#IdLink").val(element.IdLink);
