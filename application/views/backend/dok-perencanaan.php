...
<body class="site-menubar-unfold site-menubar-keep">
  ...
  <div class="page animsition">
    <div class="page-header">
        ...
        <li class="active">Pengaturan Dokumen Perencanaan</li>
      </ol>
      <h1 class="page-title">Dokumen Perencanaan</h1>
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
				...
				<iframe name="IFrameUploadFile" style="display:none;"></iframe>
			  	<form class="form-horizontal" name="FormUploadFile" id="FormUploadFile" method="POST" action="<?=base_url();?>backend/dok_perencanaan/UploadFile" enctype="multipart/form-data" target="IFrameUploadFile">
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					     	<input type="text" name="Caption" id="Caption" placeholder="Judul dokumen" class="form-control tooltip-right" title="Judul"/>					
					    </div>
					  </div>
							<div class="form-group">
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			  	...
			  	<form class="form-horizontal" name="FormFileEdit" id="FormFileEdit" method="POST">
					    ...
					    <div class="col-sm-9">
					     	<input type="text" name="Fullpath" id="Fullpath" placeholder="Lokasi" class="form-control tooltip-right" title="Lokasi" readonly/>					
					    </div>
					  </div>
						<div class="form-group">
					    <label class="col-sm-3 control-label">Opsi</label>
					 ...
					 <input type="hidden" name="IdPerencanaan" id="IdPerencanaan"/>
					<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>"/>
 ...
 <script>
	...
	$(function(){
		...
		var MyTable = $("#MyTable").dataTable({
			...
			"bServerSide": true,			
			"ajax": {
				"url": "<?=base_url();?>backend/dok_perencanaan/SliderList",
				"type": "POST",
				"data":{
					...
					buttons: {				
						...
						main: {
							...
							callback: function() {
									...
									$.ajax({
										url: '<?=base_url();?>backend/dok_perencanaan/ajax',
										type: 'POST',
										dataType: 'json',
	...
	//------------------------------------------------------------------------------------//
	//------------------------------------------------------------------------------------//	
	function FileDelete(IdPerencanaan){
		bootbox.dialog({
	...
	//------------------------------------------------------------------------------------//
	function FileEdit(IdPerencanaan){		
		$.ajax({
			url: '<?=base_url();?>backend/dok_perencanaan/ajax',
			type: 'POST',
			data: {
			...
			success: function(respon) {
				...
				if(respon.status=='sukses'){
					$.each(respon.data, function(index, element){
						...
						$("#FormFileEdit").find("#IdPerencanaan").val(element.IdPerencanaan);
