...
<body class="site-menubar-unfold site-menubar-keep">
  ...
  <div class="page animsition">
    <div class="page-header">
        ...
        <li class="active">Pengaturan Dokumen Pelaporan</li>
      </ol>
      <h1 class="page-title">Dokumen Pelaporan</h1>
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
				...
				<iframe name="IFrameUploadFile" style="display:none;"></iframe>
			  	<form class="form-horizontal" name="FormUploadFile" id="FormUploadFile" method="POST" action="<?=base_url();?>backend/dok_pelaporan/UploadFile" enctype="multipart/form-data" target="IFrameUploadFile">
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			  	...
			  	<form class="form-horizontal" name="FormFileEdit" id="FormFileEdit" method="POST">
					 ...
					 <input type="hidden" name="IdPelaporan" id="IdPelaporan"/>
					<input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>"/>
 ...
 <script>
	...
	$(function(){
		...
		var MyTable = $("#MyTable").dataTable({
			...
			"bServerSide": true,			
			"ajax": {
				"url": "<?=base_url();?>backend/dok_pelaporan/SliderList",
				"type": "POST",
				"data":{
					...
					buttons: {				
						...
						main: {
							...
							callback: function() {
									...
									$.ajax({
										url: '<?=base_url();?>backend/dok_pelaporan/ajax',
										type: 'POST',
										dataType: 'json',
	...
	//------------------------------------------------------------------------------------//
	//------------------------------------------------------------------------------------//	
	function FileDelete(IdPelaporan){
		bootbox.dialog({
			...
			success: function(respon) {
				...
				if(respon.status=='sukses'){
					$.each(respon.data, function(index, element){
						...
						$("#FormFileEdit").find("#IdPelaporan").val(element.IdPelaporan);
