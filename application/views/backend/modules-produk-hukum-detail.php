...
<body class="site-menubar-unfold site-menubar-keep">
  ...
  <div class="page animsition">
    <div class="page-header">
        ...
        <li><a href="<?=base_url();?>backend/modules_produk_hukum">Produk Hukum</a></li>
        <li class="active">Daftar Produk Hukum</li>
      </ol>
      <h1 class="page-title">Produk Hukum</h1>
			 <div class="page-header-actions">
			 
        <button class="btn btn-sm btn-icon btn-inverse" data-toggle="tooltip" 
        data-original-title="Unggah file" onclick="ProdukHukumCreate();">
    ...
    <div class="page-content">
			<div class="panel">			
				<div class="panel-body">
						...
						<div class="alert alert-info">Ada (<?=$totalprodukhukum;?>) produk hukum di kategori (<?=$namecategory;?>) ini </div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<iframe name="IFrameUploadFile" style="display:none;"></iframe>
						<form class="form-horizontal" name="FormUploadFile" id="FormUploadFile" method="POST" action="<?=base_url();?>backend/modules_produk_hukum/UploadFile" enctype="multipart/form-data" target="IFrameUploadFile">
						...
						<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" name="Nomor" id="Nomor" placeholder="Nomor" class="form-control tooltip-right" title="Nomor"/>					
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-3 control-label">Tahun</label>
								<div class="col-sm-4">
									<input type="text" name="Tahun" id="Tahun" placeholder="Tahun" class="form-control tooltip-right" title="Tahun"/>					
								</div>
							</div>
							<div class="form-group">
	...
	<div class="modal fade modal-fade-in-scale-up" id="DialogFileEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel">Update Produk Hukum</h4>
				</div>
				<div class="modal-body">
 ...
 <script>
	...
	$(function(){
		...
		var MyTable = $("#MyTable").dataTable({
			...
			"bServerSide": true,			
			"ajax": {
				"url": "<?=base_url();?>backend/modules_produk_hukum/ProdukHukumList",
				"type": "POST",
				"data":{
					'IdCategory': '<?=$idprodukhukumcategorylist;?>',	
			...
			"columnDefs": [
				 ...
				 { "orderable": true, "targets": 5, 'sClass': 'text-right' },
		...
		$("#FormFileEdit").validate({			
			...
			messages: {
				...
				Tahun: {
						...
						number: 'Tahun harus angka',
						maxlength: 'Tahun maksimal 4 angka'
				},
				Tentang: {
		...
		$("#FormFileEdit").submit(function(e){
			...
			if($("#FormFileEdit").valid()){				
				//////////////////////
				bootbox.dialog({
					message: "Apakah anda yakin akan menyimpan produk hukum ini?",
					title: "Konfirmasi",
					buttons: {				
						danger: {
							label: "No",
						...
						main: {
							...
							callback: function() {
									...
									$.ajax({
										url: '<?=base_url();?>backend/modules_produk_hukum/ajax',
										type: 'POST',
										dataType: 'json',
			...
			success: function(respon) {
				...
				if(respon.status=='sukses'){
					$.each(respon.data, function(index, element){
						$("#FormFileEdit").find("#Nomor").val(element.Nomor);
						$("#FormFileEdit").find("#Tentang").val(element.Tentang);
						$("#FormFileEdit").find("#Tahun").val(element.Tahun);
